...
df <- df[ , c("BRCA_status", "Signature.3.Whole.Genome")]
...
# Compute p-values
    ...
    wilcox.test(Signature.3.Whole.Genome ~ BRCA_status, data=subset(df, BRCA_status %in% c("BRCA-proficient", "BRCA1-deficient")))$p.value
    
    # BRCA2-deficient vs. BRCA-proficient
    wilcox.test(Signature.3.Whole.Genome ~ BRCA_status, data=subset(df, BRCA_status %in% c("BRCA-proficient", "BRCA2-deficient")))$p.value
...
# Boxplot
    ...
    data <- df
    x <- data$BRCA_status
    y <- data$Signature.3.Whole.Genome
    maintitle <- "WSI"
    xtitle <- ""
    ...
    # Sava plot
    ggsave("Figures/Figure_1A.pdf", plot, width=8, height=8)
