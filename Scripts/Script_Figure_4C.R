...
# Scatterplot (label by T.Cell.Inflamed.Signature.Score)
    ...
    x <- tsne[,1]
    y <- tsne[,2]
    z <- data$T.Cell.Inflammed.Signature.Score
    xtitle <- "tSNE1"
    ytitle <- "tSNE2"
    legend.title <- "T cell-inflamed\nsignature score"

    # Plot
    plot <- ggplot() +
        geom_point(data, mapping=aes(x=x, y=y, color=z), size=4) +
        scale_color_manual(name=legend.title, breaks=unique(z), values=c("blue", "red")) +
    ...
    # Save plot
    ggsave("Figures/Figure_4C.pdf", plot=plot, width=8, height=7)
