...
df <- df[ , c("BRCA_status", "Predicted.Neoantigens.Rooney")]
...
# Compute p-values
    ...
    wilcox.test(Predicted.Neoantigens.Rooney ~ BRCA_status, data=subset(df, BRCA_status %in% c("BRCA-proficient", "BRCA1-deficient")))$p.value
    
    # BRCA2-deficient vs. BRCA-proficient
    wilcox.test(Predicted.Neoantigens.Rooney ~ BRCA_status, data=subset(df, BRCA_status %in% c("BRCA-proficient", "BRCA2-deficient")))$p.value
...
# Boxplot
    ...
    data <- df
    x <- data$BRCA_status
    y <- log10(data$Predicted.Neoantigens.Rooney + 1)
    maintitle <- "Rooney et al."
    xtitle <- ""
    ...
    # Sava plot
    ggsave("Figures/Supp Figure_4A.pdf", plot, width=8, height=8)
