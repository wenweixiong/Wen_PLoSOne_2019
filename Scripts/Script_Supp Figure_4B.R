...
df <- df[ , c("BRCA_status", "Predicted.Neoantigens.Charoentong")]
...
# Compute p-values
    ...
    wilcox.test(Predicted.Neoantigens.Charoentong ~ BRCA_status, data=subset(df, BRCA_status %in% c("BRCA-proficient", "BRCA1-deficient")))$p.value
    
    # BRCA2-deficient vs. BRCA-proficient
    wilcox.test(Predicted.Neoantigens.Charoentong ~ BRCA_status, data=subset(df, BRCA_status %in% c("BRCA-proficient", "BRCA2-deficient")))$p.value
...
# Boxplot
    ...
    data <- df
    x <- data$BRCA_status
    y <- log10(data$Predicted.Neoantigens.Charoentong + 1)
    maintitle <- "Charoentong et al."
    xtitle <- ""
    ytitle <- "log10(Number of neoantigens)"
    ...
    # Sava plot
    ggsave("Figures/Supp Figure_4B.pdf", plot, width=8, height=8)
